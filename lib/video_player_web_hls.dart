...
import 'package:video_player_web2/video_player_web.dart';
...
class VideoPlayerPluginHls extends VideoPlayerPlatform {
  ...
  //custom code
  bool isIos = false;
  VideoPlayerPlugin videoPlayerPluginWeb = VideoPlayerPlugin();
  ...
  Future<void> init() async {
    await detectIos();
    if (isIos) {
      return videoPlayerPluginWeb.init();
    }
  ...
  Future<void> dispose(int textureId) async {
    if (isIos) {
      return videoPlayerPluginWeb.dispose(textureId);
    }
  ...
  Future<int> create(DataSource dataSource) async {
    await detectIos();
    if (isIos) {
      return videoPlayerPluginWeb.create(dataSource);
    }
  ...
  Future<void> setLooping(int textureId, bool looping) async {
    if (isIos) {
      return videoPlayerPluginWeb.setLooping(textureId, looping);
    }
  ...
  Future<void> play(int textureId) async {
    if (isIos) {
      return videoPlayerPluginWeb.play(textureId);
    }
  ...
  Future<void> pause(int textureId) async {
    if (isIos) {
      return videoPlayerPluginWeb.pause(textureId);
    }
  ...
  Future<void> setVolume(int textureId, double volume) async {
    if (isIos) {
      return videoPlayerPluginWeb.setVolume(textureId, volume);
    }
  ...
  Future<void> setPlaybackSpeed(int textureId, double speed) async {
    if (isIos) {
      return videoPlayerPluginWeb.setPlaybackSpeed(textureId, speed);
    }
    assert(speed > 0);
  ...
  Future<void> seekTo(int textureId, Duration position) async {
    if (isIos) {
      return videoPlayerPluginWeb.seekTo(textureId, position);
    }
  ...
  Future<Duration> getPosition(int textureId) async {
    if (isIos) {
      return videoPlayerPluginWeb.getPosition(textureId);
    }
  ...
  Stream<VideoEvent> videoEventsFor(int textureId) {
    if (isIos) {
      return videoPlayerPluginWeb.videoEventsFor(textureId);
    }
  ...
  Widget buildView(int textureId) {
    if (isIos) {
      return videoPlayerPluginWeb.buildView(textureId);
    }
  ...
  Future<void> setMixWithOthers(bool mixWithOthers) {
    if (isIos) {
      return videoPlayerPluginWeb.setMixWithOthers(mixWithOthers);
    }
